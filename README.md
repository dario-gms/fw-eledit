...
- **spck** — the PCK extraction tool, required for icon thumbnail loading and description editing. Place `spck.exe` in one of the following locations relative to your game root or the editor executable:
  - `<game root>/fELedit/tools/spck/spck/bin/spck.exe`
  - `<game root>/tools/spck/spck/bin/spck.exe`
  - `<game root>/tools/spck.exe`
  - `<game root>/spck.exe`
  - Same folder as `FWEledit.exe`, or any parent folder
...
3. Place `spck.exe` in one of the locations listed under [Requirements](#requirements).
4. Run `FWEledit.exe`.
...
The `configs/` folder is copied to the output directory automatically by the build rules. No NuGet packages are required — `Newtonsoft.Json` is included as source and `Ionic.Zlib` as a compiled DLL, both inside the repository.
