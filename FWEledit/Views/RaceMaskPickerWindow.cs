...
namespace FWEledit
{
    public sealed class RaceMaskPickerWindow : Form
    {
        ...
        public RaceMaskPickerWindow(uint currentValue)
        {
            SelectedValue = currentValue;

            Text = "Choose allowed races...";
        ...
        private void LoadOptions(uint currentValue)
        {
            ...
            for (int i = 0; i < RaceMaskCatalog.Options.Count; i++)
            {
                RaceMaskOption option = RaceMaskCatalog.Options[i];
                if (option == null)
                {
                    continue;
                }
                ...
                checkedListBox.SetItemChecked(index, (currentValue & option.Mask) == option.Mask);
            }
        ...
        private void UpdateSelectionDetails(int pendingIndex, CheckState pendingState)
        {
            ...
            for (int i = 0; i < checkedListBox.Items.Count; i++)
            {
                ...
                RaceMaskOption option = checkedListBox.Items[i] as RaceMaskOption;
                if (option != null)
                {
                    value |= option.Mask;
                }
            ...
            RaceMaskOption selected = checkedListBox.SelectedItem as RaceMaskOption;
