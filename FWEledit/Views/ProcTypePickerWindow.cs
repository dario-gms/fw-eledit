...
namespace FWEledit
{
    public sealed class ProcTypePickerWindow : Form
    {
        ...
        public ProcTypePickerWindow(uint currentValue)
        {
        ...
        private void LoadOptions(uint currentValue)
        {
            ...
            for (int i = 0; i < ProcTypeCatalog.Options.Count; i++)
            {
                ProcTypeOption option = ProcTypeCatalog.Options[i];
                if (option == null)
                {
                    continue;
                }
        ...
        private void UpdateSelectionDetails(int pendingIndex, CheckState pendingState)
        {
            ...
            for (int i = 0; i < checkedListBox.Items.Count; i++)
            {
                ...
                ProcTypeOption option = checkedListBox.Items[i] as ProcTypeOption;
                if (option != null)
                {
                    value |= option.Value;
                }
